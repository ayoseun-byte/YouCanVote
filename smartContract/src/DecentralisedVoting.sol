...
contract DecentralizedVoting {
    ...
    event ElectionCreated(
        uint256 indexed electionId, uint256 startTime, uint256 endTime, string title, string description
    );
    ...
    function createElection(
        ...
        elections[electionId] = Election({startTime: startTime, endTime: endTime, finalized: false, exists: true});

        emit ElectionCreated(electionId, startTime, endTime, _title, _description);
     ...
     */
    function addCandidate(uint256 _electionId, string calldata _name) external onlyAdmin electionExists(_electionId) {
     ...
     * @param _electionId Election ID
     */
    function finalizeElection(uint256 _electionId) external onlyAdmin electionExists(_electionId) {
     ...
     */
    function getElectionStatus(uint256 _electionId) external view electionExists(_electionId) returns (string memory) {
        Election memory election = elections[_electionId];
